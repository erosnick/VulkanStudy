		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "f")
				{
					...
					std::vector<unsigned int> indices;
					GenVerticesFromRawOBJ(vVerts, Positions, TCoords, Normals, indices, curline);
					...
					{
						//unsigned int indnum = (unsigned int)((Vertices.size()) - vVerts.size()) + indices[i] - 1;
						Indices.push_back(indices[i] - 1);

						//indnum = (unsigned int)((LoadedVertices.size()) - vVerts.size()) + indices[i] - 1;
						LoadedIndices.push_back(indices[i] - 1);
